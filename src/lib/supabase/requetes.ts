...
// ---------- Cartes de démonstration (jalon 2, en attendant l'import JSON du jalon 3) ----------

export async function importerCartesDemo(cartes: Carte[]): Promise<void> {
  const supabase = await createClient();

  const { error: e1 } = await supabase
    .from("subjects")
    .upsert({ id: "demo", name: "Démo : apprendre à apprendre", version: 1, color: "#4F46E5" });
  if (e1) throw new Error(e1.message);

  const moduleId = "demo/sciences-apprentissage";
  const { error: e2 } = await supabase
    .from("modules")
    .upsert({ id: moduleId, subject_id: "demo", name: "Sciences de l'apprentissage" });
  if (e2) throw new Error(e2.message);

  const concepts = [...new Map(cartes.map((c) => [c.concept_id, c.concept_nom])).entries()];
  const { error: e3 } = await supabase
    .from("concepts")
    .upsert(concepts.map(([id, name], i) => ({ id, module_id: moduleId, name, position: i })));
